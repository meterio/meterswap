...
export function useUserHasLiquidityInAllTokens(): boolean | undefined {
  ...
  const v1ExchangeLiquidityTokens = useMemo(
    () =>
      chainId ? Object.keys(exchanges).map(address => new Token(chainId, address, 18, 'MTR', 'METER MTR')) : [],
    [chainId, exchanges]
  )
...
export function useV1Trade(
  ...
  const inputIsETH = inputCurrency?.symbol === ETHER[inputPair?.chainId || 82].symbol
  const outputIsETH = outputCurrency?.symbol === ETHER[inputPair?.chainId || 82].symbol
